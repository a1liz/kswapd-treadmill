...
1. **C_warm 用 50 秒、每秒 5.1 万页的速度偷文件页，而空闲一直有 15.3GB**；
   12GB 的预热结束时缓存里只剩 3.9GB。A_warm 同样的读、同样的盘、无碎片，
   填到 13.1GB，kswapd 扫描数**恰好为 0**。
2. **`o8 = o9 = o10 = 0` 这组列是"前提"的实测**：碎片臂空闲 15.3GB，
   但**单块最大只有 256KB**（全在 o5/o6），对照臂则有几千个 o7/8/9/10 块。
   1MB 的预读 folio 在前者必然失败、在后者必然成功。
3. **两个测量窗内 `filepgMB` 都是平的**（`3886>3977`、`3980>4067`）——受害者
   跑的时候没有任何人在偷缓存；伤害在填充期就完成了。同时 kswapd 从填充期
   的 5.1 万页/秒掉到测量期的 44 页/秒：**泵就是那个顺序读**。
4. **C2 在 60 秒后重测：18.8% 命中、465 ops/s——不自愈。**
